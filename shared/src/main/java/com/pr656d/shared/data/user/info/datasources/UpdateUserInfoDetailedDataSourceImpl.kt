...
class UpdateUserInfoDetailedDataSourceImpl @Inject constructor(
    ...
    ): Event<Pair<Result<Unit>, Result<Unit>>> = coroutineScope {
        val updateBasicInfoResult = async {
            updateUserInfoBasicDataSource.updateUserInfo(userInfo)
        }

        val updateFirestoreInfoResult = async {
            updateFirestoreUserInfoDataSource.updateUserInfo(userInfo)
        }

        val result = updateBasicInfoResult.await() to updateFirestoreInfoResult.await()
