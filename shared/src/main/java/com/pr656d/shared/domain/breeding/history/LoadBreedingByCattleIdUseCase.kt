...
) : FlowUseCase<String, List<Breeding>>(ioDispatcher) {

    override fun execute(parameters: String): Flow<Result<List<Breeding>>> =
        breedingRepository.getAllBreedingByCattleId(parameters).map { Result.Success(it) }
